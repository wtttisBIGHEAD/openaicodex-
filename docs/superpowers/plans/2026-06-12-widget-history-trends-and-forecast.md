# Widget History, Trends, Forecast, and Auto Refresh Plan
...
1. 历史记录
2. 趋势小图表
3. Codex / DeepSeek 可用性估算
4. 自动刷新

## 目标

让组件能持续记录刷新结果，并基于本地历史数据展示趋势和“够不够用”的风险估算。
...
每次刷新成功后，把当前数据源的关键数据写入本地历史文件，为趋势图和预测提供基础。
...
- 手动刷新和自动刷新都写历史。
- 同一数据源 5 分钟内只保留最新一条，避免短时间重复刷新产生噪音。
- 保留最近 30 天数据，超过自动清理。
...
完整模式显示一个小型趋势图，迷你模式不显示图表。

### 显示规则

Codex：

- 默认显示最近 24 小时 `remainingPercent` 趋势。
- 后续可扩展 7 天视图；第一版先做 24 小时，避免界面过挤。
...
- 在 `index.html` 增加 `<canvas id="trendChart">` 和状态文字。
- 在 `renderer.js` 里实现轻量 Canvas 折线函数。
- 不引入图表库。
- 图表数据由主进程 IPC 提供，渲染层不读文件。
...
完整模式下放在当前 quota cards 下方或替换一部分空白区域。
...
Canvas 绘制主要手动验证；数据筛选逻辑放在 history-service 测试里覆盖。
...
- 如果数据不足：`unknown / 数据不足`
- 如果 `usedDelta <= 0` 或速度接近 0：`ok / 消耗很低，预计够用`
- 如果 `hoursUntilEmpty > hoursUntilReset`：`ok / 5小时预计够用`
- 如果 `hoursUntilEmpty <= hoursUntilReset`：`warning / 预计 X 小时后用完`

格式：

- 小于 1 小时显示分钟，例如 `预计 36 分钟后用完`
- 大于等于 1 小时显示 1 位小数，例如 `预计 1.8 小时后用完`
...
- 数据不足：`unknown / 7天数据不足`
- 消耗速度接近 0：`ok / 7天消耗很低`
- `daysUntilEmpty > daysUntilReset`：`ok / 7天预计够用`
- `daysUntilEmpty <= daysUntilReset`：`warning / 按当前速度还可用 X 天`

格式：

- 小于 1 天显示小时，例如 `还可用 9 小时`
- 大于等于 1 天显示 1 位小数，例如 `还可用 2.4 天`
...
```json
{
  ...
  "remainingPercent": 72,
  "forecast": {
    "primary": { "status": "ok", "label": "5小时预计够用", "detail": "按当前速度不会在重置前用完" },
...
让历史记录、趋势图和预测能在后台自然累积数据。

### 默认行为

- 默认每 30 分钟自动刷新一次。
- 手动刷新仍可随时触发。
- 自动刷新成功后写历史、更新趋势、更新预测。
...
完整模式当前空间有限，建议把窗口高度从 `340` 调整到 `430`。
...
新增 `history:get` IPC 和 Canvas 图表。
...
- 刚开始没有历史数据：预测和趋势图会显示数据不足，这是预期行为。
- DeepSeek 用户充值会打断余额趋势：预测算法忽略上涨段。
